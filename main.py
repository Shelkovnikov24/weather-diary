...
class ExpenseTracker:
    def __init__(self, root):
        ...
        tk.Button(frame_add, text="Добавить расход", command=self.add_expense, bg="#4CAF50", fg="white").grid(row=1, column=2, columnspan=2, pady=5, sticky="we", padx=5)
        
        # --- Фильтрация и подсчет ---
        frame_filter = tk.LabelFrame(root, text="Фильтр и аналитика")
        frame_filter.pack(padx=10, pady=5, fill="x")
        
        tk.Label(frame_filter, text="Показать категорию:").grid(row=0, column=0, padx=5)
        ...
        self.filter_category.grid(row=0, column=1, padx=5, pady=10)
        
        tk.Button(frame_filter, text="Применить фильтр", command=self.update_table).grid(row=0, column=2, padx=10)
        
        self.label_total = tk.Label(frame_filter, text="Итого по списку: 0.00 руб.", font=("Arial", 11, "bold"))
    ...
    def add_expense(self):
        ...
        # 1. Валидация суммы (проверка на положительное число)
        try:
        ...
        # 2. Валидация даты (проверка формата)
        try:
